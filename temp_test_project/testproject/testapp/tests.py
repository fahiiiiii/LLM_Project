...
def test_content_validation_success(mock_gemini_model, mock_env, mock_db_data):
        ...
        assert command.validate_content("This hotel is in a great location") == True


@pytest.mark.django_db
def test_content_validation_medical_terms(mock_gemini_model, mock_env, mock_db_data):
        command = rewrite_properties.Command()
        with pytest.raises(ValueError, match=r"Generated content contains medical terms: \['medical'\]"):
            command.validate_content("This hotel has a medical spa")

        with pytest.raises(ValueError, match=r"Generated content contains medical terms: \['patient', 'healing'\]"):
...
def test_content_validation_no_hotel_terms(mock_gemini_model, mock_env, mock_db_data):
        command = rewrite_properties.Command()
        with pytest.raises(ValueError, match=r"Generated content lacks hotel-related terms"):
...
def test_content_validation_rejects_ids(mock_gemini_model, mock_env, mock_db_data):
    command = rewrite_properties.Command()
    with pytest.raises(ValueError, match=r"Generated content contains an ID or debug-like text."):
